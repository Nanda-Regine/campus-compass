...
### Sprint 9.6 — 4-Dimension Audit & Hardening Sweep (3–4 July 2026) 🛡️ ✅
Branch `fix/ux-payments-scale-hardening` · commits `7dd61dd`, `3c0adae`, `2f9eeee`, `66d1421`, `866c39b`
Driven by a parallel 4-agent audit (gamification wiring · legibility · API · mobile), each finding verified before fix.
- [x] **Theme plumbing repaired** (root cause of most legibility bugs): next-themes uses `attribute="class"` (→ `html.outdoor/.light`) but the CSS rescue rules were keyed to `[data-theme=…]`, which never matched — so **Outdoor (bright-sunlight) mode silently rendered as dark**, and the light-mode input rescue was dead. Rekeyed every outdoor/light rule to `:is([data-theme="X"],html.X)` (ties the dark fallback specificity, wins by source order). `globals.css`
- [x] **Gamification wiring fixes**: `initXPFromDB()` was never called → new devices showed 0 XP and referral XP was invisible/overwritten (now runs once on boot in `GameLayer`). Mystery Box rewards were cosmetic (`XP_VALUES.mystery_box_opened=0`, `rolled.xp` never credited) → added `creditXP()`. Compound Day never POSTed → Domain Flames always 0; client now posts `domains_hit`, route computes XP server-side + reaches domain-streak via `req.nextUrl.origin`
- [x] **API privacy/abuse**: anonymous reports were de-anonymised (safety/incidents, community/mutual-aid, community/wisdom returned `reporter_id`/`user_id` for `is_anonymous` rows → nulled); group-chat IDOR closed (`groups/messages` GET/POST/PATCH now require `isGroupMember`); **SRC self-registration** no longer self-grants broadcast power (pending `is_active:false` + institution-admin `PATCH /api/src/members` scoped to the admin's university); mystery-box `xp_awarded` server-clamped to real loot
- [x] **Rate-limit hardening**: added caps to `nova/tts` (ElevenLabs) + `tasks/decompose` (Groq); fixed `load-shedding` window (`60ms→60s`); replaced cold-start-resettable in-memory limiters (`generate-kit`, `wellbeing/reflect`, `alumni/bridge/react`) with the shared Upstash limiter; added **daily caps** to 6 AI routes that only had per-minute; `generate-kit` bad-input now 4xx/5xx not 500; negative/overnight shift earnings guarded
- [x] **Legibility sweep** (7-agent classified pass): **336** hardcoded light-text hex `color:` values → `--text-*` tokens across ~55 files, only where the background is theme-adaptive; dark-island components + coloured-button text deliberately skipped. Verified no token landed on a solid light bg (would fail in dark)
- [x] **Theme-aware islands**: 9 components that stayed black in Sunrise/Outdoor (RoomWorkout, SexualHealthHub, SafeWalk, GBVResponseGuide, CampusRoutes, CulturalMovement, RegulationRoom, WisdomArchive, DietaryPreferencesModal) converted to `var(--bg-*)` roots + `--text-*` (15 bg + 177 text). Dark mode unchanged (`--bg-base=#05040C`); light/outdoor now adapt
- [x] **Study pages pop on navy**: `.study-page` dark-mode-scoped lift of `--text-muted`/`--text-tertiary` + low-alpha white labels
- [x] **Mobile**: nav-clear "Start Workout" CTA; 9 keyboard-safe modals (`85dvh`+scroll); enlarged sub-40px tap targets; raised ~135 sub-0.6rem fonts to a 0.62rem floor
- All batches TypeScript clean. **Not yet visually QA'd** in a browser (offline sandbox) — theme toggle (Dark→Sunrise→Outdoor) needs an eyeball pass. Next migration still `000042`.
