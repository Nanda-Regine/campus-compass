    ...
    <div style={{ paddingBottom: 80 }}>
      ...
      <div style={{ padding: '20px 20px 0' }}>
        ...
        <div style={{ display: 'flex', gap: 4, marginBottom: 14 }}>
          {TABS.map(t => (
            <button key={t.id} onClick={() => setTab(t.id)} style={{ padding: '6px 14px', borderRadius: 20, border: '0.5px solid', borderColor: tab === t.id ? '#4ecf9e' : 'rgba(255,255,255,0.1)', background: tab === t.id ? 'rgba(78,207,158,0.12)' : 'rgba(255,255,255,0.04)', color: tab === t.id ? '#4ecf9e' : 'rgba(255,255,255,0.45)', fontFamily: 'DM Sans, sans-serif', fontSize: '0.75rem', fontWeight: tab === t.id ? 600 : 400, cursor: 'pointer' }}>
              {t.label}
            </button>
          ))}
        </div>
        ...
        {tab === 'browse' && (
          <div style={{ position: 'relative', marginBottom: 14 }}>
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search by subject or module" style={{ width: '100%', paddingLeft: 34, paddingRight: 12, paddingTop: 10, paddingBottom: 10, background: 'rgba(255,255,255,0.05)', border: '0.5px solid rgba(255,255,255,0.1)', borderRadius: 10, color: 'var(--text-primary)', fontSize: '0.82rem', fontFamily: 'DM Sans, sans-serif', outline: 'none', boxSizing: 'border-box' as const }} />
          </div>
        )}
        ...
        {tab === 'sessions' && (
          <div style={{ display: 'flex', gap: 4, marginBottom: 14 }}>
            {(['student', 'tutor'] as const).map(r => (
              <button key={r} onClick={() => setSessionRole(r)} style={{ padding: '5px 12px', borderRadius: 20, border: '0.5px solid', borderColor: sessionRole === r ? '#c9a84c' : 'rgba(255,255,255,0.1)', background: sessionRole === r ? 'rgba(201,168,76,0.1)' : 'rgba(255,255,255,0.04)', color: sessionRole === r ? '#c9a84c' : 'rgba(255,255,255,0.4)', fontFamily: 'DM Sans, sans-serif', fontSize: '0.72rem', cursor: 'pointer' }}>
                As {r}
              </button>
            ))}
          </div>
      ...
      <div style={{ padding: '0 20px' }}>
        {tab === 'browse' ? (
          loading ? (
            Array.from({ length: 3 }).map((_, i) => <div key={i} style={{ height: 130, borderRadius: 14, background: 'rgba(255,255,255,0.04)', marginBottom: 10 }} />)
          ) : tutors.length === 0 ? (
        ...
        <div style={{ position: 'fixed', inset: 0, zIndex: 60, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'flex-end', backdropFilter: 'blur(6px)' }}>
          <div style={{ width: '100%', background: 'var(--bg-surface)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '20px 20px 0 0', padding: '20px 20px 40px' }}>
            ...
            <div style={{ display: 'flex', gap: 8, marginBottom: 14 }}>
              {[1,2,3,4,5].map(n => (
                <button key={n} onClick={() => setReviewRating(n)} style={{ flex: 1, padding: '10px 0', borderRadius: 10, border: '0.5px solid', borderColor: reviewRating >= n ? '#f59e0b' : 'rgba(255,255,255,0.1)', background: reviewRating >= n ? 'rgba(245,158,11,0.1)' : 'rgba(255,255,255,0.04)', fontSize: '1.2rem', cursor: 'pointer' }}>
                  ⭐
                </button>
              ))}
            </div>
            <textarea value={reviewComment} onChange={e => setReviewComment(e.target.value)} placeholder="How was the session? (optional)" style={{ width: '100%', padding: '10px 12px', background: 'rgba(255,255,255,0.05)', border: '0.5px solid rgba(255,255,255,0.12)', borderRadius: 10, color: 'var(--text-primary)', fontSize: '0.82rem', fontFamily: 'DM Sans, sans-serif', outline: 'none', resize: 'none', height: 80, boxSizing: 'border-box', marginBottom: 12 }} />
            <div style={{ display: 'flex', gap: 8 }}>
              <button onClick={() => setReviewingSession(null)} style={{ flex: 1, padding: '12px', borderRadius: 12, border: '0.5px solid rgba(255,255,255,0.1)', background: 'rgba(255,255,255,0.04)', color: 'rgba(255,255,255,0.5)', fontFamily: 'DM Sans, sans-serif', fontSize: '0.82rem', cursor: 'pointer' }}>Cancel</button>
    ...
    <div style={{ background: 'var(--bg-surface)', border: '1px solid rgba(78,207,158,0.12)', borderRadius: 16, padding: 14, marginBottom: 10, position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 10 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ fontFamily: 'DM Sans, sans-serif', fontSize: '0.7rem', color: 'rgba(255,255,255,0.4)', marginTop: 2 }}>
      ...
      {tutor.bio && (
        <div style={{ fontFamily: 'DM Sans, sans-serif', fontSize: '0.73rem', color: 'rgba(255,255,255,0.4)', marginBottom: 10, lineHeight: 1.4 }}>
      ...
      {tutor.availability && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 5, marginBottom: 10 }}>
          ...
          <span style={{ fontFamily: 'DM Sans, sans-serif', fontSize: '0.68rem', color: 'rgba(255,255,255,0.35)' }}>{tutor.availability}</span>
        </div>
      )}
    ...
    <div style={{ background: 'var(--bg-surface)', border: `1px solid ${color}22`, borderRadius: 16, padding: 14, marginBottom: 10, position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
        <div>
          ...
          <div style={{ fontFamily: 'DM Sans, sans-serif', fontSize: '0.7rem', color: 'rgba(255,255,255,0.4)', marginTop: 2 }}>{other}</div>
        </div>
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: session.notes ? 8 : 10 }}>
        {session.scheduled_date && (
          <span style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '0.6rem', color: 'rgba(255,255,255,0.4)' }}>📅 {session.scheduled_date}</span>
        )}
      ...
      {session.notes && (
        <div style={{ fontFamily: 'DM Sans, sans-serif', fontSize: '0.7rem', color: 'rgba(255,255,255,0.35)', marginBottom: 10, lineHeight: 1.4 }}>
          {session.notes}
        </div>
      )}
