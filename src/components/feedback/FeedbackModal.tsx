    ...
    >
      {showReviewPrompt ? (
        ...
        <div className="space-y-5">
          {/* Star rating */}
          <div>
            <label className="font-mono text-[0.62rem] text-white uppercase tracking-wide mb-2 block">
              ...
              {[1, 2, 3, 4, 5].map(star => (
                <button
                  ...
                  className={cn(
                    ...
                    rating >= star ? 'text-amber-400' : 'text-white'
                  )}
                >
                  ★
                </button>
          ...
          <div>
            ...
            <textarea
              ...
              rows={4}
              maxLength={1000}
              className="w-full bg-[var(--bg-surface)] border border-white/10 focus:border-teal-600 focus:ring-2 focus:ring-teal-600/20 rounded-xl px-4 py-3 text-sm text-white placeholder:text-white resize-none outline-none transition-all font-body"
            />
            <div className="text-right font-mono text-[0.63rem] text-white mt-1">
              {message.length}/1000
            </div>
          </div>
        </div>
