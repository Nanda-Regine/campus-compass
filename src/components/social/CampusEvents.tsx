    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          {institution && (
            <div className="font-mono text-[0.65rem] text-white/78 mt-0.5">{institution}</div>
          )}
        </div>
        <button
          ...
          <div className="grid grid-cols-2 gap-2">
            <div className="col-span-2">
              <input
                ...
                className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-sm text-white placeholder:text-white/75 outline-none focus:border-emerald-500 font-body"
              />
            </div>
            <div>
              <label className="font-mono text-[0.65rem] text-white/80 mb-1 block">Type</label>
            ...
            <div>
              <label className="font-mono text-[0.65rem] text-white/80 mb-1 block">Venue</label>
              <input value={fVenue} onChange={e => setFVenue(e.target.value)} placeholder="e.g. Library Room 2"
                className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-sm text-white placeholder:text-white/75 outline-none focus:border-emerald-500 font-body" />
            </div>
            <div>
              <label className="font-mono text-[0.65rem] text-white/80 mb-1 block">Date & time *</label>
            ...
            <div>
              <label className="font-mono text-[0.65rem] text-white/80 mb-1 block">Duration (minutes)</label>
              <input type="number" value={fDuration} onChange={e => setFDuration(e.target.value)} placeholder="e.g. 90" min={15}
                className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-sm text-white placeholder:text-white/75 outline-none focus:border-emerald-500 font-body" />
            </div>
            <div>
              <label className="font-mono text-[0.65rem] text-white/80 mb-1 block">Max attendees</label>
            ...
            <div className="col-span-2">
              <textarea value={fDesc} onChange={e => setFDesc(e.target.value)} placeholder="Description (optional)" rows={2}
                className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-sm text-white placeholder:text-white/75 outline-none focus:border-emerald-500 resize-none font-body" />
            </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-10">
          ...
          <div className="font-mono text-[0.6rem] text-white/78 mt-1">Be the first to post one above!</div>
        </div>
      ) : (
        <div className="space-y-3">
          {events.map(e => {
              ...
              <div key={e.id} className="bg-white/3 border border-white/7 rounded-2xl p-4 transition-all hover:border-white/12"
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    ...
                    {e.description && (
                      <div className="font-mono text-[0.6rem] text-white/80 leading-relaxed mt-1 line-clamp-2">
                        {e.description}
                      </div>
                    )}
                  </div>
                  ...
                  <div className="flex flex-col items-end gap-1.5 flex-shrink-0">
                    <button
                      ...
                      className={cn(
                        ...
                        e.user_rsvped
                          ? 'bg-emerald-600/20 text-emerald-400 border-emerald-600/30'
                          : isFull
                          ? 'bg-white/3 text-white/72 border-white/8 cursor-not-allowed'
                    ...
                    {e.rsvp_count > 0 && (
                      <span className="font-mono text-[0.65rem] text-white/75">{e.rsvp_count} going</span>
                    )}
                    {isOwner && (
