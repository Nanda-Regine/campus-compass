...
export default function SignupForm() {
  ...
  const onSubmit = async (data: FormData) => {
    setAuthError(null)
    const { error, session } = await signUp(data.email, data.password, data.name, true)
    if (error) { setAuthError(error); return }
    // Email auto-confirm ON → signUp returns a live session, so email is never in
    // the critical path (survives a 1000-user burst / email outage). Go straight
    // to onboarding; middleware keeps them on /setup until it's done. Only fall
    // back to the "check your email" screen if confirmation is re-enabled later.
    if (session) {
      router.push('/setup')
      router.refresh()
    } else {
  ...
  if (submitted) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center px-5 text-center" style={{ background: 'var(--bg-base)' }}>
        ...
        <p className="font-body text-sm text-white max-w-xs leading-relaxed mb-6">
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ background: 'var(--bg-base)', position: 'relative', overflow: 'hidden' }}>
      ...
      <div className="px-5 pt-12 pb-8 text-center">
        ...
        <p className="font-mono text-xs text-white">Free forever. No credit card needed.</p>
      </div>

      <div className="flex-1 px-5 pb-10 max-w-sm mx-auto w-full">
        <div className="card-base" style={{ padding: 24 }}>

          {/* Google is the promoted primary path: pre-verified, sends zero email,
              so it can't be throttled by the mail provider during a launch burst. */}
          <button
            type="button"
            onClick={handleGoogle}
            disabled={googleLoading}
            className="w-full inline-flex items-center justify-center gap-2.5 rounded-xl bg-white px-4 py-3 font-display font-bold text-[0.9rem] text-[#1f1f1f] shadow-teal transition-all duration-150 hover:bg-white/90 active:scale-[0.97] disabled:opacity-60"
          >
            {googleLoading ? (
              <svg className="animate-spin h-4 w-4 text-[#1f1f1f]" viewBox="0 0 24 24" fill="none">
          ...
          <p className="mt-2 mb-5 text-center font-mono text-[0.6rem] text-teal-400">
            Fastest — no email confirmation needed
          </p>

          <div className="flex items-center gap-3 mb-5">
            <div className="flex-1 h-px bg-white/7" />
            <span className="font-mono text-[0.6rem] text-white uppercase tracking-widest">or use email</span>
            <div className="flex-1 h-px bg-white/7" />
          </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} noValidate className="space-y-4">
            ...
            <div ref={consentRef} className="rounded-xl border border-white/8 bg-white/3 px-3 py-3">
              <label className="flex items-start gap-3 cursor-pointer">
                ...
                />
                <span className="font-mono text-[0.6rem] text-white leading-relaxed">
        ...
        <p className="text-center mt-5 font-mono text-xs text-white">
