...
function MacroRing({ value, target, color, label, unit }: {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4, minWidth: 60 }}>
      ...
      <div style={{ fontFamily: 'var(--font-mono)', fontSize: '0.62rem', color: '#fff', textAlign: 'center', lineHeight: 1.4 }}>
        {label}<br />
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <div className="font-mono text-[0.65rem] text-white">{today}</div>
          {logs.length > 0 && (
            <div
      ...
      <div className="bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded-2xl p-4">
        ...
        <div className="mb-4">
          <div className="flex items-center justify-between mb-1.5">
            <div className="font-mono text-[0.65rem] text-white uppercase tracking-wide">Calories</div>
          ...
          {totals.calories === 0 && (
            <div className="font-mono text-[0.63rem] text-white mt-1">Log your first meal below to track your day</div>
          )}
        </div>
          ...
          <div key={slot} className="bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded-2xl overflow-hidden">
            ...
            <div className="flex items-center justify-between px-4 py-3 border-b border-white/5">
              <div>
                ...
                {slotCal > 0 && (
                  <span className="ml-2 font-mono text-[0.63rem] text-white">{slotCal} kcal</span>
                )}
              </div>
              <button
                ...
                className={cn(
                  ...
                  isOpen
                    ...
                    : 'bg-[var(--bg-base)] border-white/10 hover:border-teal-600/50 text-white hover:text-teal-400'
                )}
              >
                {isOpen ? '↑ Close' : '+ Add'}
              </button>
                ...
                {slotLogs.map(log => (
                  <div key={log.id} className="flex items-center gap-3 px-4 py-2.5 group">
                    <div className="flex-1 min-w-0">
                      <div className="font-body text-sm text-white truncate">{log.food_name}</div>
            ...
            {slotLogs.length === 0 && !isOpen && (
              <div className="px-4 py-3">
                <div className="font-mono text-[0.65rem] text-white">Nothing logged yet — tap + Add</div>
              </div>
            )}
            ...
            {isOpen && (
              <div className="border-t border-white/8 p-3 space-y-3 animate-fade-up">
                <div className="font-mono text-[0.63rem] text-white uppercase tracking-widest">SA food quick-add</div>
                  ...
                  {foods.map(food => (
                    ...
                    >
                      ...
                      <div className="min-w-0">
                        <div className="font-body text-[0.72rem] text-white leading-tight truncate">{food.name}</div>
                        <div className="font-mono text-[0.65rem] text-white flex gap-2">
                          <span>{food.cal} kcal</span>
                          {food.protein > 5 && <span className="text-rose-400">P{Math.round(food.protein)}g</span>}
                          {'price' in food && <span className="text-emerald-400">~R{(food as typeof food & {price:number}).price}</span>}
                        </div>
                      </div>
                    </button>
                  ))}
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 bg-[var(--bg-base)] border border-white/10 focus:border-teal-600 rounded-lg px-3 py-2 text-sm text-white placeholder:text-white outline-none transition-all"
                  />
                  <input
                    type="number"
                    value={customCal}
                    ...
                    placeholder="kcal"
                    className="w-16 bg-[var(--bg-base)] border border-white/10 focus:border-teal-600 rounded-lg px-2 py-2 text-sm text-white placeholder:text-white outline-none"
                  />
                  <button
