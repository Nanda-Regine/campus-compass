...
export default function FoodSpendTracker({ userId, foodBudget }: Props) {
  ...
  if (loading) return (
    <div style={{ display: 'flex', justifyContent: 'center', padding: '40px 0' }}>
      <span style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: '#fff' }}>Loading food spend…</span>
    </div>
  )
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      {foodBudget > 0 ? (
        <div style={card}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
            <div>
              <p style={{ fontFamily: 'var(--font-mono)', fontSize: 9.5, color: '#fff', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 4 }}>
                Food budget — {today.slice(0, 7)}
              </p>
              <p style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 22, color: '#fff', lineHeight: 1 }}>
                R{totalSpent.toFixed(2)}
                <span style={{ fontFamily: 'var(--font-mono)', fontWeight: 400, fontSize: 11, color: '#fff', marginLeft: 6 }}>/ R{foodBudget}</span>
              </p>
            </div>
            {remaining != null && (
              <div style={{ textAlign: 'right' }}>
                ...
                <p style={{ fontFamily: 'var(--font-mono)', fontSize: 9.5, color: '#fff' }}>remaining</p>
              </div>
            )}
          </div>
          ...
          <div style={{ display: 'flex', gap: 16 }}>
            <div>
              <p style={{ fontFamily: 'var(--font-mono)', fontSize: 9.5, color: '#fff', marginBottom: 2 }}>Today</p>
              <p style={{ fontFamily: 'var(--font-display)', fontWeight: 600, fontSize: 13, color: '#fff' }}>R{todayTotal.toFixed(0)}</p>
            </div>
            {dailyLeft != null && (
              <div>
                <p style={{ fontFamily: 'var(--font-mono)', fontSize: 9.5, color: '#fff', marginBottom: 2 }}>Daily allowance left</p>
        ...
        <div style={{ ...card, background: 'rgba(245,158,11,0.06)', border: '1px solid rgba(245,158,11,0.15)' }}>
          <p style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: 'rgb(245,158,11)', marginBottom: 4 }}>
            💡 Set a food budget in Budget OS to see your progress bar
          </p>
          <p style={{ fontFamily: 'var(--font-mono)', fontSize: 9.5, color: '#fff' }}>
      ...
      {/* ── Quick-log buttons ────────────────────────────────────────────── */}
      <div style={card}>
        <p style={{ fontFamily: 'var(--font-mono)', fontSize: 9.5, color: '#fff', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 12 }}>
        ...
        {showCustom ? (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            <div style={{ display: 'flex', gap: 8 }}>
              <input
                ...
                style={{ flex: 1, background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 10, padding: '8px 12px', color: '#fff', fontFamily: 'var(--font-body)', fontSize: 13, outline: 'none' }}
              />
              <div style={{ display: 'flex', alignItems: 'center', background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 10, padding: '0 10px', gap: 4 }}>
                <span style={{ fontFamily: 'var(--font-mono)', fontSize: 12, color: '#fff' }}>R</span>
                <input
                  type="number"
                  inputMode="decimal"
                  value={customAmt}
                  ...
                  onKeyDown={e => e.key === 'Enter' && handleCustom()}
                  placeholder="0"
                  style={{ width: 54, background: 'transparent', border: 'none', color: '#fff', fontFamily: 'var(--font-mono)', fontSize: 13, outline: 'none' }}
                />
              </div>
            </div>
            <div style={{ display: 'flex', gap: 8 }}>
              <button onClick={() => setShowCustom(false)} style={{ flex: 1, padding: '8px', borderRadius: 10, border: '1px solid rgba(255,255,255,0.1)', background: 'transparent', color: '#fff', fontFamily: 'var(--font-mono)', fontSize: 11, cursor: 'pointer' }}>
                Cancel
              </button>
          ...
          <button
            ...
            style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, padding: '9px', borderRadius: 10, border: '1px dashed rgba(255,255,255,0.15)', background: 'transparent', color: '#fff', fontFamily: 'var(--font-mono)', fontSize: 11, cursor: 'pointer' }}
          >
      ...
      {sortedDates.length > 0 ? (
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
                    {byDate[date].map(e => (
                      <div key={e.id} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        ...
                        <span style={{ flex: 1, fontFamily: 'var(--font-body)', fontSize: 12.5, color: '#fff', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                          {e.description}
                        </span>
                        <span style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: '#4ecf9e', flexShrink: 0 }}>
                          R{e.amount.toFixed(0)}
                        </span>
                        <button onClick={() => deleteExpense(e.id)} style={{ flexShrink: 0, background: 'none', border: 'none', cursor: 'pointer', color: '#fff', padding: '2px 4px' }}>
                          <Trash2 size={11} />
                        </button>
                      </div>
                    ))}
        ...
        <div style={{ ...card, textAlign: 'center', padding: '28px 20px' }}>
          <Utensils size={28} style={{ color: '#fff', margin: '0 auto 10px' }} />
          <p style={{ fontFamily: 'var(--font-display)', fontSize: 13, color: '#fff', marginBottom: 6 }}>No meals logged this month</p>
          <p style={{ fontFamily: 'var(--font-mono)', fontSize: 9.5, color: '#fff' }}>Use the quick buttons above to log your first meal</p>
        </div>
      )}

      {/* ── Budget stretch tip ───────────────────────────────────────────── */}
      <div style={{ background: 'rgba(78,207,158,0.06)', border: '1px solid rgba(78,207,158,0.14)', borderRadius: 12, padding: '12px 14px' }}>
        <p style={{ fontFamily: 'var(--font-mono)', fontSize: 9.5, color: 'rgb(78,207,158)', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 6 }}>
          💡 Budget stretch tip
        </p>
      ...
      {foodBudget > 0 && pct > 85 && (
        <div style={{ background: 'rgba(239,68,68,0.08)', border: '1px solid rgba(239,68,68,0.2)', borderRadius: 12, padding: '12px 14px', display: 'flex', alignItems: 'flex-start', gap: 10 }}>
          ...
          <div>
            <p style={{ fontFamily: 'var(--font-display)', fontWeight: 600, fontSize: 12.5, color: '#fff', marginBottom: 3 }}>
              {pct >= 100 ? 'Food budget exceeded' : 'Food budget almost gone'}
            </p>
            <p style={{ fontFamily: 'var(--font-mono)', fontSize: 9.5, color: '#fff', lineHeight: 1.5 }}>
              {pct >= 100
