    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="space-y-2">
        <div className="grid grid-cols-2 gap-2">
          <input
            ...
            className="col-span-2 w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-sm text-white placeholder:text-white outline-none focus:border-purple-500 font-body"
          />
        </div>
        ...
        <div className="relative">
          <textarea
            ...
            rows={7}
            className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2.5 text-sm text-white placeholder:text-white outline-none focus:border-purple-500 resize-none font-body leading-relaxed"
          />
          {/* Character progress */}
      ...
      {kit && (
        <div className="space-y-5">
          ...
          <div className="bg-purple-500/6 border border-purple-500/20 rounded-2xl p-4">
            ...
            <p className="font-body text-sm text-white leading-relaxed">{kit.summary}</p>
          </div>

          {/* Key Concepts */}
          {kit.key_concepts.length > 0 && (
            <div className="bg-white/3 border border-white/7 rounded-2xl p-4">
              <div className="font-mono text-[0.63rem] text-white tracking-widest mb-3">KEY CONCEPTS</div>
          ...
          {kit.flashcards.length > 0 && (
            <div className="bg-white/3 border border-white/7 rounded-2xl p-4">
              <div className="flex items-center justify-between mb-3">
                <div className="font-mono text-[0.63rem] text-white tracking-widest">
                  FLASHCARDS ({kit.flashcards.length})
                </div>
                <span className="font-mono text-[0.65rem] text-white">tap to flip</span>
              </div>
              <div className="space-y-2">
              ...
              {!saved ? (
                <div className="mt-4 pt-3 border-t border-white/8 space-y-2">
                  <div className="font-mono text-[0.63rem] text-white tracking-widest">SAVE TO FLASHCARD DECK</div>
                  <select
                    value={deckId}
                    onChange={e => setDeckId(e.target.value)}
                    className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-sm text-white outline-none focus:border-purple-500 font-body"
                  >
                  ...
                  {deckId === '__new__' && (
                    <input
                      ...
                      className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-sm text-white placeholder:text-white outline-none focus:border-purple-500 font-body"
                    />
                  )}
                  <button
          ...
          {kit.quiz.length > 0 && (
            <div className="bg-white/3 border border-white/7 rounded-2xl p-4">
              <div className="flex items-center justify-between mb-3">
                <div className="font-mono text-[0.63rem] text-white tracking-widest">QUIZ</div>
                  ...
                  <div className="space-y-3">
                        ...
                        return (
                          <button
                            ...
                            className={cn(
                              ...
                              !showResult && 'bg-white/3 border-white/10 hover:border-white/20 text-white',
                              ...
                              showResult && !isPicked && !isRight && 'bg-white/2 border-white/5 text-white',
                            )}
                          >
                            {opt}
                    ...
                    {isRevealedQ && (
                      <div className="bg-white/3 border border-white/8 rounded-xl px-3 py-2">
                        <span className="font-mono text-[0.65rem] text-white block mb-0.5">EXPLANATION</span>
                        <span className="font-body text-xs text-white leading-relaxed">{q.explanation}</span>
                      </div>
                    )}
                    ...
                    <div className="flex gap-2 pt-1">
                      <button
                        ...
                        disabled={quizIdx === 0}
                        className="flex-1 font-mono text-[0.6rem] py-2 rounded-xl border border-white/10 text-white disabled:opacity-25 hover:border-white/20 transition-all"
                      >
                        ← Prev
                      </button>
                      <button
          ...
          <button
            ...
            className="w-full font-mono text-[0.6rem] py-2 rounded-xl border border-white/10 text-white hover:text-white hover:border-white/20 transition-all"
          >
      ...
      {!kit && !generating && (
        <div className="bg-white/2 border border-white/6 rounded-2xl p-4 space-y-2">
          <div className="font-mono text-[0.63rem] text-white tracking-widest">TIPS FOR BEST RESULTS</div>
          <ul className="space-y-1.5">
            {[
              ...
              <li key={i} className="flex items-start gap-2">
                ...
                <span className="font-mono text-[0.65rem] text-white leading-relaxed">{tip}</span>
              </li>
            ))}
          </ul>
        </div>
