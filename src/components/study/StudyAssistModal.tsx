    ...
    <Modal open={open} onClose={onClose} title={modalTitle} size="lg">
      {loading && (
        <div className="flex flex-col items-center justify-center py-12 gap-4">
          ...
          <p className="font-mono text-[0.62rem] text-white">Nova is thinking…</p>
        </div>
      )}
      ...
      {type === 'grade_calculator' && !submitted && !loading && (
        <div className="space-y-4">
          ...
          <div>
            <label className="font-mono text-[0.6rem] text-white uppercase tracking-wide block mb-1.5">Module name</label>
            <input
              value={gradeModuleName}
              onChange={e => setGradeModuleName(e.target.value)}
              placeholder="e.g. Organic Chemistry"
              className="w-full bg-[var(--bg-surface)] border border-white/10 focus:border-teal-600 rounded-xl px-3 py-2.5 text-sm text-white placeholder:text-white outline-none transition-all"
            />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="font-mono text-[0.6rem] text-white uppercase tracking-wide block mb-1.5">Current grade (%)</label>
              <input
                type="number"
                min="0"
                max="100"
            ...
            <div>
              <label className="font-mono text-[0.6rem] text-white uppercase tracking-wide block mb-1.5">Target grade (%)</label>
              <input
                type="number"
                min="0"
                max="100"
          ...
          <div>
            <label className="font-mono text-[0.6rem] text-white uppercase tracking-wide block mb-1.5">
                  ...
                  {plan?.dailyPlan?.map((day, i) => (
                    <div key={i} className="bg-[var(--bg-surface)] border border-teal-600/20 rounded-xl p-4">
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <span className="font-mono text-[0.65rem] text-white">{day.hours}h</span>
                      </div>
                      <p className="font-body text-sm text-white mb-2">{day.focus}</p>
                      <ul className="space-y-1">
                        {day.tasks.map((t, j) => (
                          <li key={j} className="font-mono text-[0.6rem] text-white flex gap-1.5">
                            <span className="text-teal-600 flex-shrink-0">·</span>{t}
                          </li>
                        ))}
                      </ul>
                      {day.tip && (
                        <p className="font-mono text-[0.65rem] text-teal-400 mt-2 italic">💡 {day.tip}</p>
                      )}
                    </div>
                  ))}
                </div>
                {plan?.motivationNote && (
                  <p className="font-body text-sm text-white italic text-center py-2">{plan.motivationNote}</p>
                )}
              </>
            )
          })()}
            ...
            return (
              <>
                {guide?.prepPhases && guide.prepPhases.length > 0 && (
                  <div className="space-y-3">
                    <div className="font-mono text-[0.65rem] text-white uppercase tracking-widest">Prep Phases</div>
                    {guide.prepPhases.map((phase, i) => (
                      <div key={i} className="bg-[var(--bg-surface)] border border-purple-500/20 rounded-xl p-4">
                        <div className="flex items-start justify-between gap-2 mb-1">
                          ...
                          <span className="font-mono text-[0.63rem] text-white flex-shrink-0">{phase.days}</span>
                        </div>
                        <p className="font-body text-sm text-white mb-2">{phase.focus}</p>
                        ...
                        <p className="font-mono text-[0.65rem] text-white mt-2">✓ {phase.milestone}</p>
                      </div>
                    ))}
                  </div>
                )}
                {guide?.studyTechniques && guide.studyTechniques.length > 0 && (
                  <div>
                    <div className="font-mono text-[0.65rem] text-white uppercase tracking-widest mb-2">Study Techniques</div>
                    <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-none">
                      {guide.studyTechniques.map((t, i) => (
                        <div key={i} className="flex-shrink-0 bg-[var(--bg-surface)] border border-white/7 rounded-xl p-3 w-40">
                          ...
                          <p className="font-mono text-[0.63rem] text-white leading-relaxed">{t.description}</p>
                        </div>
                      ))}
                    </div>
                  </div>
                      ...
                      <div className="bg-[var(--bg-surface)] border border-white/7 rounded-xl p-3">
                        ...
                        <ul className="space-y-1.5">
                          {guide.dayBeforeTips.map((tip, i) => (
                            <li key={i} className="font-mono text-[0.65rem] text-white flex gap-1.5"><span>·</span>{tip}</li>
                          ))}
                        </ul>
                      </div>
                    )}
            ...
            return (
              <>
                <div className="text-center py-4">
                  ...
                  <div className="font-mono text-[0.6rem] text-white mb-3">Required on remaining assessment</div>
                ...
                {res?.explanation && (
                  <div className="bg-[var(--bg-surface)] border border-white/7 rounded-xl p-4">
                    <p className="font-body text-sm text-white leading-relaxed">{res.explanation}</p>
                  </div>
                )}
                {res?.advice && (
                  <p className="font-body text-sm text-teal-400 leading-relaxed">{res.advice}</p>
                )}
                {res?.alternativeScenarios && res.alternativeScenarios.length > 0 && (
                  <div>
                    <div className="font-mono text-[0.65rem] text-white uppercase tracking-widest mb-2">Alternative targets</div>
                    <div className="flex gap-2">
                      {res.alternativeScenarios.map((s, i) => (
                        <div key={i} className="flex-1 bg-[var(--bg-surface)] border border-white/7 rounded-xl p-3 text-center">
                          ...
                          <div className="font-mono text-[0.63rem] text-white mt-0.5">{s.label}</div>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                <button
                  ...
                  className="w-full font-mono text-xs text-white hover:text-white py-2 transition-colors"
                >
              ...
              <>
                {analysis?.overallLoad && (
                  <div className="flex items-center gap-3">
                    <span className="font-mono text-[0.65rem] text-white uppercase tracking-widest">Overall load:</span>
                    <span className={cn(
                ...
                {analysis?.topPriority && (
                  <div className="bg-[var(--bg-surface)] border border-white/7 rounded-xl px-4 py-3">
                    <div className="font-mono text-[0.63rem] text-white uppercase tracking-widest mb-1">Top priority</div>
                ...
                {analysis?.conflictPeriods && analysis.conflictPeriods.length > 0 && (
                  <div className="space-y-3">
                    <div className="font-mono text-[0.65rem] text-white uppercase tracking-widest">Conflict periods</div>
                      ...
                      <div key={i} className={cn('rounded-xl p-4 border', severityStyle[period.severity] || severityStyle.low)}>
                        ...
                        <ul className="space-y-0.5 mb-2">
                          {period.items.map((item, j) => (
                            <li key={j} className="font-mono text-[0.65rem] text-white">· {item}</li>
                          ))}
                        </ul>
                        <p className="font-mono text-[0.6rem] text-white">{period.suggestion}</p>
                      </div>
                    ))}
                  </div>
                )}
