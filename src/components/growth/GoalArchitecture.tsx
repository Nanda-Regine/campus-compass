...
interface Sprint{id:number;goal:string;startDate:string;endDate:string;milestones:string[];completed:boolean}
...
function SprintTab({state,update}:{state:GoalState;update:(s:GoalState)=>void}) {
  ...
  const add=()=>{
    ...
    const s:Sprint={id:Date.now(),goal:form.goal,startDate:form.start,endDate:form.end,milestones,completed:false}
      ...
      {activeSprint?(
        <div style={{background:'var(--bg-surface)',border:'1px solid rgba(99,102,241,0.25)',borderRadius:14,padding:'16px',display:'flex',flexDirection:'column',gap:10}}>
          ...
          {activeSprint.milestones.length>0&&(
            <div>
              <div style={{fontSize:'0.62rem',fontFamily:'var(--font-mono)',color:'var(--text-muted)',marginBottom:6}}>MILESTONES</div>
              {activeSprint.milestones.map((m,i)=><div key={i} style={{fontSize:'0.75rem',color:'var(--text-secondary)',padding:'4px 0',borderBottom:'1px solid var(--border-subtle)'}}>• {m}</div>)}
            </div>
          )}
