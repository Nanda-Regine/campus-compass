...
export default function MoodCheckin({ userId }: Props) {
  ...
  if (todayScore !== null && !showGratitude) {
    ...
    return (
      <div className="flex items-center gap-2 px-1">
        <span className="font-mono text-[0.58rem] text-white/50 uppercase tracking-widest">Today</span>
        <span className="text-base">{mood?.emoji}</span>
        <span className="font-mono text-[0.58rem] text-white/60">{mood?.label}</span>
      </div>
    )
  }
  ...
  return (
    <div className="bg-[var(--bg-surface)] border border-white/10 rounded-2xl px-4 py-3.5">
      <div className="font-mono text-[0.58rem] text-white/55 uppercase tracking-widest mb-3">
      ...
      <div className="flex items-center justify-between gap-1">
        {MOODS.map(mood => (
          ...
          >
            ...
            <span className="font-mono text-[0.65rem] text-white/50">{mood.label}</span>
          </button>
        ))}
      </div>
    </div>
