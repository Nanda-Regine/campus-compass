...
export default function JobApplicationTracker({ userId }: { userId: string }) {
  ...
  if (loading) return (
    <div style={{ display: 'flex', justifyContent: 'center', padding: '40px 0' }}>
      <span style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: 'rgba(255,255,255,0.25)' }}>Loading applications…</span>
    </div>
  )
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <div style={{ display: 'flex', gap: 10 }}>
        {[
          ...
          <div key={s.label} style={{ flex: 1, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 12, padding: '10px 12px', textAlign: 'center' }}>
            <p style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 22, color: s.color, lineHeight: 1, marginBottom: 3 }}>{s.value}</p>
            <p style={{ fontFamily: 'var(--font-mono)', fontSize: 9.5, color: 'rgba(255,255,255,0.35)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>{s.label}</p>
          </div>
        ))}
      </div>
      ...
      {adding && (
        <div style={{ ...card, display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          {[
            ...
            <div key={key}>
              <label style={{ display: 'block', fontFamily: 'var(--font-mono)', fontSize: 9.5, color: 'rgba(255,255,255,0.35)', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 5 }}>{label}</label>
              <input
                value={(form as Record<string, string>)[key]}
                onChange={e => setForm(p => ({ ...p, [key]: e.target.value }))}
                placeholder={placeholder}
                style={{ width: '100%', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 10, padding: '9px 12px', color: 'rgba(255,255,255,0.85)', fontFamily: 'var(--font-body)', fontSize: 13, outline: 'none', boxSizing: 'border-box' }}
              />
            </div>
          ))}
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            <div style={{ flex: 1 }}>
              <label style={{ display: 'block', fontFamily: 'var(--font-mono)', fontSize: 9.5, color: 'rgba(255,255,255,0.35)', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 5 }}>Deadline</label>
              <input type="date" value={form.deadline} onChange={e => setForm(p => ({ ...p, deadline: e.target.value }))}
                style={{ width: '100%', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 10, padding: '9px 12px', color: 'rgba(255,255,255,0.85)', fontFamily: 'var(--font-mono)', fontSize: 12, outline: 'none', boxSizing: 'border-box', colorScheme: 'dark' }} />
            </div>
            <div style={{ flex: 1 }}>
              <label style={{ display: 'block', fontFamily: 'var(--font-mono)', fontSize: 9.5, color: 'rgba(255,255,255,0.35)', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 5 }}>Applied date</label>
          ...
          {/* Status */}
          <div>
            <label style={{ display: 'block', fontFamily: 'var(--font-mono)', fontSize: 9.5, color: 'rgba(255,255,255,0.35)', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 8 }}>Status</label>
          ...
          {/* Job type */}
          <div>
            <label style={{ display: 'block', fontFamily: 'var(--font-mono)', fontSize: 9.5, color: 'rgba(255,255,255,0.35)', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 8 }}>Type</label>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
              {(Object.keys(JOB_TYPE_LABELS) as JobType[]).map(t => (
                <button key={t} onClick={() => setForm(p => ({ ...p, job_type: t }))} style={{
                  ...
                  color: form.job_type === t ? '#4ecf9e' : 'rgba(255,255,255,0.45)',
                  fontFamily: 'var(--font-mono)', fontSize: 10.5, cursor: 'pointer',
                }}>
                  {JOB_TYPE_LABELS[t]}
                </button>
          ...
          {(['all', ...STATUS_ORDER] as const).map(s => {
            ...
            return (
              <button key={s} onClick={() => setFilter(s)} style={{
                ...
                color: active ? (m?.color ?? '#94a3b8') : 'rgba(255,255,255,0.4)',
                fontFamily: 'var(--font-mono)', fontSize: 10.5, cursor: 'pointer',
              }}>
      ...
      {visible.length === 0 && !adding ? (
        <div style={{ ...card, textAlign: 'center', padding: '32px 20px' }}>
          ...
          <p style={{ fontFamily: 'var(--font-display)', fontSize: 14, color: 'rgba(255,255,255,0.4)', marginBottom: 6 }}>
            ...
            <div key={app.id} style={{ background: 'rgba(255,255,255,0.04)', border: `1px solid ${isOpen ? 'rgba(255,255,255,0.14)' : 'rgba(255,255,255,0.07)'}`, borderRadius: 14, overflow: 'hidden' }}>
              ...
              >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, alignItems: 'center' }}>
                    <span style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: 'rgba(255,255,255,0.5)' }}>{app.company}</span>
                ...
                <div style={{ flexShrink: 0, color: 'rgba(255,255,255,0.3)', marginTop: 3 }}>
              ...
              {isOpen && (
                <div style={{ borderTop: '1px solid rgba(255,255,255,0.07)', padding: '14px 14px', display: 'flex', flexDirection: 'column', gap: 14 }}>
                  ...
                  {/* Pipeline stepper */}
                  <div>
                    <p style={{ fontFamily: 'var(--font-mono)', fontSize: 9.5, color: 'rgba(255,255,255,0.3)', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 10 }}>Update status</p>
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                      {STATUS_ORDER.map(s => {
                  ...
                  {(app.status === 'phone_screen' || app.status === 'interview' || app.status === 'assessment') && (
                    <div>
                      <label style={{ display: 'block', fontFamily: 'var(--font-mono)', fontSize: 9.5, color: 'rgba(255,255,255,0.35)', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 5 }}>
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, alignItems: 'center' }}>
                    {app.location && (
                      <span style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: 'rgba(255,255,255,0.4)' }}>📍 {app.location}</span>
                    )}
                    {app.salary_range && (
                      <span style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: '#4ecf9e' }}>💰 {app.salary_range}</span>
                    )}
                    {app.applied_date && (
                      <span style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: 'rgba(255,255,255,0.35)' }}>📤 Applied {app.applied_date}</span>
                    )}
                    {app.url && (
