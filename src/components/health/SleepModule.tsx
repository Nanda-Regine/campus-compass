      ...
      {activeTab === 'log' && (
        <div className="space-y-4">
          ...
          <div className="bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded-2xl p-4 space-y-4">
            <div className="font-mono text-[0.62rem] text-white uppercase tracking-widest">
              {editId ? 'Update today\'s sleep' : 'Log last night\'s sleep'}
            </div>

            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="font-mono text-[0.62rem] text-white block mb-1.5">Bed time</label>
                <input
                  type="time"
                  value={bedtime}
              ...
              <div>
                <label className="font-mono text-[0.62rem] text-white block mb-1.5">Wake time</label>
                <input
                  type="time"
                  value={wakeTime}
            ...
            {bedtime && wakeTime && (
              <div className="flex items-center gap-2">
                <div className="font-mono text-[0.6rem] text-white">Duration:</div>
                <div
                ...
                {sleepHours(bedtime, wakeTime) < 6 && (
                  <div className="font-mono text-[0.62rem] text-rose-400">Too short — cognitive function drops significantly below 6h</div>
                )}
              </div>
            )}
            ...
            {/* Quality */}
            <div>
              <label className="font-mono text-[0.62rem] text-white block mb-2">Sleep quality</label>
            ...
            {/* Notes */}
            <div>
              <label className="font-mono text-[0.62rem] text-white block mb-1.5">Notes (optional)</label>
              <input
                value={notes}
                onChange={e => setNotes(e.target.value)}
                placeholder="e.g. stressed about exam, woke up twice"
                className="w-full bg-[var(--bg-base)] border border-white/10 focus:border-indigo-500 rounded-xl px-3.5 py-2.5 text-sm text-white placeholder:text-white outline-none transition-all"
              />
            </div>
          ...
          {recentLogs.length > 0 && (
            <div>
              <div className="font-mono text-[0.62rem] text-white uppercase tracking-widest mb-2">Recent nights</div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <button
                        ...
                        className="opacity-0 group-hover:opacity-100 text-white hover:text-red-400 transition-all text-xs flex-shrink-0"
                      >
                        ✕
                      </button>
                    </div>
        ...
        <div className="space-y-4">
          {recentLogs.length === 0 ? (
            <div className="text-center py-12">
              ...
              <div className="font-mono text-[0.6rem] text-white">Log a few nights to see your stats</div>
            </div>
          ) : (
            <>
              {/* Summary stats */}
              <div className="grid grid-cols-3 gap-3">
                <div className="bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded-xl p-3 text-center">
                  ...
                  <div className="font-mono text-[0.62rem] text-white mt-0.5">Avg / night</div>
                </div>
                <div className="bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded-xl p-3 text-center">
                  ...
                  <div className="font-mono text-[0.62rem] text-white mt-0.5">Sleep debt</div>
                </div>
                <div className="bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded-xl p-3 text-center">
                  <div className="font-display font-black text-lg text-indigo-400">{recentLogs.length}</div>
                  <div className="font-mono text-[0.62rem] text-white mt-0.5">Nights logged</div>
                </div>
              </div>

              {/* 7-day bar chart */}
              <div className="bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded-2xl p-4">
                <div className="font-mono text-[0.62rem] text-white uppercase tracking-widest mb-3">Last 7 nights</div>
                  ...
                  {recentLogs.slice(0, 7).reverse().map(log => {
                    ...
                    return (
                      <div key={log.id} className="flex-1 flex flex-col items-center gap-1">
                        <div className="font-mono text-[0.65rem] text-white">{h}h</div>
                        <div
                        ...
                        />
                        <div className="font-mono text-[0.62rem] text-white text-center leading-tight">{date}</div>
                      </div>
                    )
                  })}
                </div>
                <div className="flex items-center gap-3 mt-3">
                  <div className="flex items-center gap-1.5">
                    ...
                    <div className="font-mono text-[0.65rem] text-white">7h+ (goal)</div>
                  </div>
                  <div className="flex items-center gap-1.5">
                    <div className="w-2 h-2 rounded-full bg-amber-400" />
                    <div className="font-mono text-[0.65rem] text-white">6–7h</div>
                  </div>
                  <div className="flex items-center gap-1.5">
                    <div className="w-2 h-2 rounded-full bg-rose-400" />
                    <div className="font-mono text-[0.65rem] text-white">&lt;6h</div>
                  </div>
                </div>
              </div>
              ...
              {sleepDebt > 2 && (
                ...
                >
                  <div className="font-mono text-[0.62rem] text-rose-400 uppercase tracking-widest">
                    ⚠️ Sleep debt: {sleepDebt.toFixed(1)}h
                  </div>
                  <div className="font-body text-sm text-white leading-relaxed">
              ...
              {/* Exam tips */}
              <div className="bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded-2xl p-4">
                <div className="font-mono text-[0.62rem] text-indigo-400 uppercase tracking-widest mb-3">🧠 Sleep science for exams</div>
                  ...
                  {[
                    ...
                    <div key={item.tip} className="flex gap-3">
                      ...
                      <div>
                        ...
                        <div className="font-body text-[0.78rem] text-white leading-relaxed mt-0.5">{item.detail}</div>
                      </div>
                    </div>
                  ))}
                </div>
          ...
          {quizDone ? (
            <>
              ...
              >
                ...
                <div className="font-body text-sm text-white leading-relaxed max-w-xs mx-auto">{chronotype.desc}</div>
              </div>

              {/* Score breakdown */}
              <div className="bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded-2xl p-4">
                <div className="font-mono text-[0.62rem] text-white uppercase tracking-widest mb-3">Your chronotype spectrum</div>
              ...
              <button
                ...
                className="w-full font-mono text-[0.6rem] text-white hover:text-white py-2 transition-colors"
              >
                Retake quiz
              </button>
            </>
            ...
            <>
              <div className="bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded-2xl p-4">
                ...
                <div className="font-body text-sm text-white mb-4">
                  ...
                  {QUIZ_QUESTIONS.map((q, qi) => (
                    <div key={qi}>
                      <div className="font-display font-bold text-sm text-white mb-2">
                        <span className="font-mono text-[0.62rem] text-indigo-400 mr-2">{qi + 1}.</span>
                        {q.q}
                      </div>
                          ...
                          <button
                            ...
                            className={cn(
                              ...
                              answers[qi] === oi
                                ...
                                : 'bg-[var(--bg-base)] border-white/8 text-white hover:border-white/15'
                            )}
                          >
                            {opt}
                          </button>
