...
interface WorkoutLog{id:number;date:string;type:string;duration:number;notes:string}
interface FitnessState{logs:WorkoutLog[];streak:number;lastWorkout:string|null}

const STORAGE_KEY='varsityos-fitness'
function load():FitnessState{if(typeof window==='undefined')return{logs:[],streak:0,lastWorkout:null};try{return JSON.parse(localStorage.getItem(STORAGE_KEY)||'null')||{logs:[],streak:0,lastWorkout:null}}catch{return{logs:[],streak:0,lastWorkout:null}}}
function save(s:FitnessState){localStorage.setItem(STORAGE_KEY,JSON.stringify(s))}
...
export default function FitnessTracker() {
  ...
  const [form,setForm]=useState({type:'run',duration:'30',notes:'',date:new Date().toISOString().split('T')[0]})

  const update=(s:FitnessState)=>{setState(s);save(s)}

  const logWorkout=()=>{
    const today=form.date
    const log:WorkoutLog={id:Date.now(),date:today,type:form.type,duration:parseInt(form.duration)||30,notes:form.notes}
    const logs=[log,...state.logs].slice(0,90)
    ...
    update({logs,streak,lastWorkout:today})
    setAdding(false)
    setForm({type:'run',duration:'30',notes:'',date:new Date().toISOString().split('T')[0]})
  }

  const thisWeek=state.logs.filter(l=>{const d=new Date(l.date);const now=new Date();const diff=(now.getTime()-d.getTime())/86400000;return diff<7})
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',gap:14}}>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:8}}>
        {[{l:'Streak',v:`${state.streak}d`,c:'var(--rose,#FB7185)'},{l:'This week',v:`${thisWeek.length} sessions`,c:'var(--teal)'},{l:'Weekly mins',v:`${totalMins}m`,c:totalMins>=150?'var(--teal)':totalMins>=90?'var(--gold)':'var(--coral)'}].map(s=>(
      ...
      {totalMins<150&&<div style={{padding:'8px 12px',background:'rgba(251,113,133,0.06)',border:'1px solid rgba(251,113,133,0.2)',borderRadius:8,fontSize:'0.7rem',color:'var(--rose,#FB7185)'}}>
      ...
      {tab==='log'&&(
        <div style={{display:'flex',flexDirection:'column',gap:10}}>
          {adding?(
            <div style={{background:'var(--bg-elevated)',border:'1px solid var(--border-default)',borderRadius:12,padding:'14px',display:'flex',flexDirection:'column',gap:10}}>
              ...
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}>
                <div><div style={{fontSize:'0.65rem',color:'var(--text-muted)',marginBottom:3}}>Date</div><input type="date" value={form.date} onChange={e=>setForm(v=>({...v,date:e.target.value}))} style={{width:'100%',padding:'8px 10px',background:'var(--bg-base)',border:'1px solid var(--border-default)',borderRadius:7,color:'var(--text-primary)',fontSize:'0.78rem'}}/></div>
                <div><div style={{fontSize:'0.65rem',color:'var(--text-muted)',marginBottom:3}}>Duration (min)</div><input type="number" value={form.duration} onChange={e=>setForm(v=>({...v,duration:e.target.value}))} style={{width:'100%',padding:'8px 10px',background:'var(--bg-base)',border:'1px solid var(--border-default)',borderRadius:7,color:'var(--text-primary)',fontSize:'0.82rem',fontFamily:'var(--font-mono)'}}/></div>
              </div>
              ...
              <div style={{display:'flex',gap:8}}>
                <button onClick={logWorkout} style={{flex:1,padding:'10px 0',background:'rgba(251,113,133,0.1)',border:'1px solid rgba(251,113,133,0.25)',borderRadius:9,color:'var(--rose,#FB7185)',fontSize:'0.75rem',fontFamily:'var(--font-mono)',fontWeight:700,cursor:'pointer'}}>Log workout ✓</button>
                <button onClick={()=>setAdding(false)} style={{padding:'10px 14px',background:'transparent',border:'1px solid var(--border-subtle)',borderRadius:9,color:'var(--text-tertiary)',fontSize:'0.73rem',cursor:'pointer'}}>Cancel</button>
              </div>
            </div>
          ):(
            <button onClick={()=>setAdding(true)} style={{padding:'12px 0',background:'rgba(251,113,133,0.07)',border:'1px solid rgba(251,113,133,0.2)',borderRadius:10,color:'var(--rose,#FB7185)',fontSize:'0.78rem',fontFamily:'var(--font-mono)',fontWeight:700,cursor:'pointer'}}>+ Log today&apos;s workout</button>
          )}
          ...
          {state.logs.length===0&&<div style={{textAlign:'center',padding:'24px',color:'var(--text-muted)',fontSize:'0.75rem'}}>No workouts logged yet. Start with 20 minutes today.</div>}
        </div>
      )}
