  ...
  if (view === 'detail' && selected) {
    ...
    return (
      <div className="flex flex-col gap-4">
        <button onClick={() => { setView('list'); setContribs([]) }} className="font-mono text-[0.62rem] text-white/35 hover:text-white/60 self-start">
          ← All circles
        </button>
        ...
        <div className="bg-white/3 border border-emerald-500/20 rounded-2xl p-4" style={{ background: 'linear-gradient(135deg, rgba(52,211,153,0.06) 0%, transparent 70%)' }}>
          ...
          {selected.description && <div className="font-mono text-[0.62rem] text-white/40 mt-1">{selected.description}</div>}
        ...
        <div className="bg-white/3 border border-white/7 rounded-2xl p-4 space-y-3">
          <div className="flex items-center justify-between">
            <div className="font-mono text-[0.6rem] text-white/40 uppercase tracking-wide">Round {round}</div>
          ...
          <div>
            <div className="font-mono text-[0.56rem] text-white/25 uppercase mb-2">{paidCount}/{totalCount} contributed</div>
            ...
            {loadingC ? (
              <div className="font-mono text-[0.62rem] text-white/30 text-center py-4">Loading…</div>
            ) : (
                  ...
                  return (
                    <div key={m.id} className={cn(
                      ...
                      <div className="flex items-center gap-2.5">
                        <div className={cn('w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold font-display flex-shrink-0', hasPaid ? 'bg-emerald-500 text-white' : 'bg-white/10 text-white/40')}>
                      ...
                      {hasPaid && isCreator && (
                        <button
                          ...
                          className="font-mono text-[0.48rem] text-white/20 hover:text-red-400 transition-colors flex-shrink-0"
                        >
                          undo
                        </button>
                      )}
        ...
        <div className="bg-white/3 border border-white/7 rounded-2xl p-4">
          <div className="font-mono text-[0.6rem] text-white/40 uppercase tracking-wide mb-3">Payout schedule</div>
            ...
            {members.map((m, i) => {
              ...
              return (
                <div key={m.id} className={cn('flex items-center gap-3 rounded-lg px-2.5 py-2', isCurrent ? 'bg-emerald-500/8 border border-emerald-500/15' : isPast ? 'opacity-40' : 'bg-white/2')}>
                  <div className={cn('font-mono text-[0.55rem] w-5 text-center font-bold', isCurrent ? 'text-emerald-400' : isPast ? 'text-white/25' : 'text-white/35')}>
                    {roundNum}
                  </div>
        ...
        {selected.stokvel_circle_members.some(m => m.status === 'invited') && (
          <div className="bg-white/3 border border-white/7 rounded-2xl p-4">
            <div className="font-mono text-[0.6rem] text-white/40 uppercase tracking-wide mb-2">Pending invites</div>
            <div className="space-y-1.5">
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="bg-emerald-500/6 border border-emerald-500/20 rounded-2xl p-4" style={{ background: 'linear-gradient(135deg, rgba(52,211,153,0.06) 0%, transparent 70%)' }}>
        ...
        <div className="font-mono text-[0.62rem] text-white/45 leading-relaxed">
      ...
      {showNew ? (
        <div className="bg-white/3 border border-white/10 rounded-2xl p-4 space-y-3">
          ...
          <input value={newName} onChange={e => setNewName(e.target.value)} placeholder="Circle name (e.g. Res Floor Stokvel)" className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-sm text-white placeholder:text-white/25 outline-none focus:border-emerald-500 font-body" />
          <textarea value={newDesc} onChange={e => setNewDesc(e.target.value)} placeholder="Description (optional)" rows={2} className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-sm text-white placeholder:text-white/25 outline-none focus:border-emerald-500 resize-none font-body" />
          ...
          <div>
            <label className="font-mono text-[0.55rem] text-white/35 mb-2 block">Invite members (they&apos;ll get a join link)</label>
            <div className="space-y-2">
              {memberRows.map((r, i) => (
              ...
              <button onClick={() => setMemberRows([...memberRows, { email: '', name: '' }])} className="font-mono text-[0.58rem] text-white/30 hover:text-teal-400 transition-colors">
                + Add another member
              </button>
            </div>
          </div>
          ...
          <div className="flex gap-2 pt-1">
            ...
            <button onClick={() => setShowNew(false)} className="px-4 font-mono text-sm text-white/40 border border-white/10 rounded-xl">Cancel</button>
          </div>
        </div>
      ) : (
      ...
      {stokvels.length === 0 && !showNew ? (
        <div className="text-center py-8">
          ...
          <div className="font-mono text-[0.62rem] text-white/30 mt-1">Start one above or ask a friend to invite you.</div>
        </div>
      ) : (
              ...
              >
                <div className="flex items-start justify-between gap-2 mb-2">
                  ...
                  <div className="flex flex-col items-end gap-1">
                    ...
                    <div className="font-mono text-[0.48rem] text-white/25">pot size</div>
                  </div>
                </div>
