  ...
  if (view === 'detail' && selected) {
    ...
    return (
      <div className="flex flex-col gap-4">
        <button onClick={() => { setView('list'); setContribs([]) }} className="font-mono text-[0.62rem] text-white/60 hover:text-white/60 self-start">
          ← All circles
        </button>
        ...
        <div className="bg-white/3 border border-emerald-500/20 rounded-2xl p-4" style={{ background: 'linear-gradient(135deg, rgba(52,211,153,0.06) 0%, transparent 70%)' }}>
          ...
          {selected.description && <div className="font-mono text-[0.62rem] text-white/65 mt-1">{selected.description}</div>}
          <div className="grid grid-cols-3 gap-2 mt-3">
            <div className="text-center">
              ...
              <div className="font-mono text-[0.65rem] text-white/55 uppercase">{FREQ_LABEL[selected.frequency]}</div>
            </div>
            <div className="text-center">
              <div className="font-display font-black text-white text-base">{members.length}</div>
              <div className="font-mono text-[0.65rem] text-white/55 uppercase">members</div>
            </div>
            <div className="text-center">
              <div className="font-display font-black text-amber-400 text-base">R{potSize.toFixed(0)}</div>
              <div className="font-mono text-[0.65rem] text-white/55 uppercase">pot size</div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white/3 border border-white/7 rounded-2xl p-4 space-y-3">
          <div className="flex items-center justify-between">
            <div className="font-mono text-[0.6rem] text-white/65 uppercase tracking-wide">Round {round}</div>
          ...
          <div>
            <div className="font-mono text-[0.56rem] text-white/50 uppercase mb-2">{paidCount}/{totalCount} contributed</div>
            ...
            {loadingC ? (
              <div className="font-mono text-[0.62rem] text-white/55 text-center py-4">Loading…</div>
            ) : (
                  ...
                  return (
                    <div key={m.id} className={cn(
                      ...
                      <div className="flex items-center gap-2.5">
                        <div className={cn('w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold font-display flex-shrink-0', hasPaid ? 'bg-emerald-500 text-white' : 'bg-white/10 text-white/65')}>
                          {hasPaid ? '✓' : m.payout_position}
                        </div>
                        <div>
                          <div className="flex items-center gap-1.5">
                            <span className="font-display font-bold text-white text-xs">{m.display_name || m.email}</span>
                            {isMe && <span className="font-mono text-[0.48rem] bg-white/8 text-white/60 px-1 rounded">you</span>}
                          ...
                          <div className="font-mono text-[0.65rem] text-white/55 mt-0.5">
                      ...
                      {hasPaid && isCreator && (
                        <button
                          ...
                          className="font-mono text-[0.48rem] text-white/45 hover:text-red-400 transition-colors flex-shrink-0"
                        >
                          undo
                        </button>
                      )}
        ...
        <div className="bg-white/3 border border-white/7 rounded-2xl p-4">
          <div className="font-mono text-[0.6rem] text-white/65 uppercase tracking-wide mb-3">Payout schedule</div>
            ...
            {members.map((m, i) => {
              ...
              return (
                <div key={m.id} className={cn('flex items-center gap-3 rounded-lg px-2.5 py-2', isCurrent ? 'bg-emerald-500/8 border border-emerald-500/15' : isPast ? 'opacity-40' : 'bg-white/2')}>
                  <div className={cn('font-mono text-[0.55rem] w-5 text-center font-bold', isCurrent ? 'text-emerald-400' : isPast ? 'text-white/50' : 'text-white/60')}>
                    {roundNum}
                  </div>
                  <div className="flex-1 min-w-0">
                    <span className="font-display font-bold text-white text-xs">{m.display_name || m.email}</span>
                  </div>
                  <div className={cn('font-mono text-[0.65rem]', isCurrent ? 'text-emerald-400' : isPast ? 'text-white/45' : 'text-white/55')}>
                    {isPast ? 'received' : isCurrent ? 'this round' : `round ${roundNum}`}
                  </div>
                  <div className="font-mono text-[0.55rem] text-white/65">R{potSize.toFixed(0)}</div>
                </div>
              )
            })}
          </div>
        ...
        {selected.stokvel_circle_members.some(m => m.status === 'invited') && (
          <div className="bg-white/3 border border-white/7 rounded-2xl p-4">
            <div className="font-mono text-[0.6rem] text-white/65 uppercase tracking-wide mb-2">Pending invites</div>
            <div className="space-y-1.5">
                ...
                return (
                  <div key={m.id} className="flex items-center justify-between bg-white/2 border border-white/5 rounded-xl px-3 py-2">
                    <div>
                      ...
                      <div className="font-mono text-[0.65rem] text-white/50">position #{m.payout_position}</div>
                    </div>
                    <button
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="bg-emerald-500/6 border border-emerald-500/20 rounded-2xl p-4" style={{ background: 'linear-gradient(135deg, rgba(52,211,153,0.06) 0%, transparent 70%)' }}>
        ...
        <div className="font-mono text-[0.62rem] text-white/68 leading-relaxed">
      ...
      {showNew ? (
        <div className="bg-white/3 border border-white/10 rounded-2xl p-4 space-y-3">
          ...
          <input value={newName} onChange={e => setNewName(e.target.value)} placeholder="Circle name (e.g. Res Floor Stokvel)" className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-sm text-white placeholder:text-white/50 outline-none focus:border-emerald-500 font-body" />
          <textarea value={newDesc} onChange={e => setNewDesc(e.target.value)} placeholder="Description (optional)" rows={2} className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-sm text-white placeholder:text-white/50 outline-none focus:border-emerald-500 resize-none font-body" />
          <div className="grid grid-cols-2 gap-2">
            <div>
              <label className="font-mono text-[0.55rem] text-white/60 mb-1 block">Contribution amount (R) *</label>
              <input type="number" inputMode="decimal" value={newAmount} onChange={e => setNewAmount(e.target.value)} placeholder="e.g. 200" min={10} className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-sm text-white placeholder:text-white/50 outline-none focus:border-emerald-500 font-body" />
            </div>
            <div>
              <label className="font-mono text-[0.55rem] text-white/60 mb-1 block">Frequency</label>
          ...
          <div>
            <label className="font-mono text-[0.55rem] text-white/60 mb-2 block">Invite members (they&apos;ll get a join link)</label>
            <div className="space-y-2">
              {memberRows.map((r, i) => (
                <div key={i} className="grid grid-cols-2 gap-2">
                  <input type="email" value={r.email} onChange={e => { const next = [...memberRows]; next[i] = { ...next[i], email: e.target.value }; setMemberRows(next) }} placeholder={`Email ${i+1}`} className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-1.5 text-xs text-white placeholder:text-white/45 outline-none focus:border-emerald-500 font-body" />
                  <input value={r.name} onChange={e => { const next = [...memberRows]; next[i] = { ...next[i], name: e.target.value }; setMemberRows(next) }} placeholder="Name (optional)" className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-1.5 text-xs text-white placeholder:text-white/45 outline-none focus:border-emerald-500 font-body" />
                </div>
              ))}
              <button onClick={() => setMemberRows([...memberRows, { email: '', name: '' }])} className="font-mono text-[0.58rem] text-white/55 hover:text-teal-400 transition-colors">
                + Add another member
              </button>
            </div>
          </div>
          ...
          <div className="flex gap-2 pt-1">
            ...
            <button onClick={() => setShowNew(false)} className="px-4 font-mono text-sm text-white/65 border border-white/10 rounded-xl">Cancel</button>
          </div>
        </div>
      ) : (
      ...
      {stokvels.length === 0 && !showNew ? (
        <div className="text-center py-8">
          ...
          <div className="font-mono text-[0.62rem] text-white/55 mt-1">Start one above or ask a friend to invite you.</div>
        </div>
      ) : (
              ...
              >
                <div className="flex items-start justify-between gap-2 mb-2">
                  ...
                  <div className="flex flex-col items-end gap-1">
                    ...
                    <div className="font-mono text-[0.48rem] text-white/50">pot size</div>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  <div className={cn('font-mono text-[0.65rem] px-1.5 py-0.5 rounded border', s.status === 'active' ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : 'bg-white/5 text-white/55 border-white/10')}>
                    {s.status}
                  </div>
                  <div className="font-mono text-[0.65rem] text-white/50">Round {s.current_round}</div>
