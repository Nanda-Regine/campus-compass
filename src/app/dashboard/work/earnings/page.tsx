...
export default function EarningsPage() {
  ...
  return (
    <div className="min-h-screen pb-24" style={{ background: 'var(--bg-base)', position: 'relative', overflow: 'hidden' }}>
      <AmbientImage zone="budget" opacity={0.34} blurPx={6} saturation={1.4} overlayColor="linear-gradient(180deg,rgba(5,4,12,0.55) 0%,rgba(5,4,12,0.64) 100%)" />
      ...
      <div className="px-4 py-3 space-y-4 max-w-2xl mx-auto">
        ...
        >
          <div className="font-mono text-[0.6rem] text-teal-400 uppercase tracking-widest mb-2">Total earned (all time)</div>
          <div className="font-display font-black text-3xl text-white">
            {fmt.currencyShort(totalEarnings)}
          </div>
          <div className="font-mono text-[0.65rem] text-teal-300 mt-1">
        ...
        {thisMonth && thisMonth.earnings > 0 && (
          <div className="bg-[var(--bg-surface)] border border-white/7 rounded-2xl p-4">
            <div className="font-mono text-[0.65rem] text-white uppercase tracking-widest mb-2">
              {thisMonth.label}
            </div>
            ...
            <div className="mt-3 pt-3 border-t border-white/7">
              <div className="font-mono text-[0.65rem] text-white mb-1.5">Approximate coverage</div>
              <div className="space-y-1">
                {getWhatItCovers(thisMonth.earnings).map(item => (
                  <div key={item.label} className="flex items-center justify-between text-xs">
                    <span className="font-body text-white">{item.emoji} {item.label}</span>
                    <span className={`font-mono font-bold ${item.covered ? 'text-teal-400' : 'text-white'}`}>
        ...
        {/* ─── Month history ─── */}
        <section>
          <div className="font-mono text-[0.6rem] text-white uppercase tracking-widest mb-2.5">Monthly history</div>
          {loading ? (
            ...
            <div className="text-center py-8">
              ...
              <p className="font-mono text-[0.6rem] text-white">No worked shifts yet — mark shifts as worked to track earnings</p>
            </div>
          ) : (
            <div className="space-y-2">
              {sorted.map(m => (
