...
export async function POST(req: Request) {
  ...
  try {
    ...
    const rl = await checkRateLimitAsync(user.id, 'shift-draft', 10, 60_000)
...
Keep it under 5 sentences. Professional but human. Ready to copy to WhatsApp.`,
    ...
    console.error('[shift-draft] error:', error)
    return NextResponse.json({ error: 'Failed to generate shift request. Please try again.' }, { status: 500 })
  }
}
