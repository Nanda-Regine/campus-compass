...
export async function POST(request: NextRequest) {
  try {
    ...
    const rateCheck = await checkRateLimitAsync(user.id, 'dashboard-coach-summary', 3, 3_600_000)
    ...
    try {
      ...
      console.error('[dashboard/coach-summary] JSON parse failed. Raw:', clean.slice(0, 200))
