...
export async function POST(req: Request) {
  ...
  const { type, title, description, location, latitude, longitude, severity, institution } = body as Record<string, unknown>
  const { data, error } = await supabase
    .from('safety_incidents')
    .insert({ type, title, description, location, latitude, longitude, severity, institution, reporter_id: user.id })
    .select()
    .single()
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })
