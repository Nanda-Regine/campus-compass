    ...
    >
      ...
      <p
        style={{
          ...
          color: '#fff', textAlign: 'center', marginBottom: 20, lineHeight: 1.6,
          maxWidth: 280,
        }}
      >
        {cards.length > 0
        ...
        >
          {cards.map((card, i) => (
            ...
            >
              ...
              <div style={{ fontFamily: 'var(--font-mono)', fontSize: '0.65rem', color: '#fff', marginBottom: 2, textTransform: 'uppercase', letterSpacing: '0.07em' }}>
                {card.label}
              </div>
          ...
          >
            ...
            <div className="flex items-center justify-between px-4 py-4 border-b border-white/10">
              <div>
                ...
                <div className="font-mono text-[0.68rem] text-white mt-0.5">{historyList.length} conversations</div>
              </div>
              <div className="flex items-center gap-2">
                ...
                <button
                  onClick={() => setShowHistory(false)}
                  className="w-7 h-7 rounded-lg bg-white/5 border border-white/10 flex items-center justify-center text-white hover:text-white transition-all text-xs"
                >
                  ✕
                </button>
              </div>
            ...
            <div className="flex-1 overflow-y-auto py-2">
              {historyLoading ? (
                ...
                <div className="px-4 py-8 text-center">
                  ...
                  <p className="font-mono text-[0.72rem] text-white">No conversations yet</p>
                  <p className="font-mono text-[0.68rem] text-white mt-1">Start chatting with Nova</p>
                </div>
              ) : (
                    ...
                    >
                      <div className="flex items-start justify-between gap-2">
                        <div className="min-w-0 flex-1">
                          ...
                          <div className="flex items-center gap-2">
                            <span className="font-mono text-[0.65rem] text-white">{convo.message_count} msgs</span>
                            <span className="font-mono text-[0.65rem] text-white">·</span>
                            <span className="font-mono text-[0.65rem] text-white">{dateStr}</span>
                          </div>
                        </div>
                        <button
                          onClick={(e) => deleteConversation(convo.id, e)}
                          className="opacity-0 group-hover:opacity-100 w-6 h-6 rounded flex items-center justify-center text-white hover:text-red-400 hover:bg-red-500/10 transition-all text-xs flex-shrink-0"
                          title="Delete"
                        >
                          ✕
                        </button>
      ...
      <TopBar
        ...
        action={
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={openHistory}
              className="font-mono text-[0.7rem] px-2.5 py-1 rounded-lg border border-white/10 bg-white/5 text-white hover:text-white hover:bg-white/10 transition-all"
              title="Chat history"
            >
              ☰ History
            </button>
            ...
            {!isUnlimited && !isPremium && (
              <div className="flex items-center gap-1.5">
                ...
                <span className={cn('font-mono text-[0.68rem]',
                  usagePercent > 80 ? 'text-red-400' : usagePercent > 50 ? 'text-amber-400' : 'text-white'
                )}>
                  {usageLeft} left
                </span>
              </div>
              ...
              <div className="space-y-1.5">
                {CRISIS_NUMBERS.map(c => (
                  <div key={c.name} className="flex items-center gap-2">
                    <span className="font-mono text-[0.72rem] text-white w-16">{c.type}</span>
                  ...
                  <div
                    className={cn(
                      ...
                      msg.role === 'user'
                        ...
                        : 'bg-[var(--bg-surface)] border border-white/7 text-white rounded-tl-sm'
                    )}
                  >
                    ...
                    <div className={cn(
                      ...
                      <div className={cn(
                        ...
                        msg.role === 'user' ? 'text-teal-400' : 'text-white'
                      )}>
                        ...
                        <button
                          ...
                          className={cn(
                            ...
                            speakingMsgId === msg.id
                              ...
                              : 'bg-white/4 text-white hover:bg-white/10 hover:text-white border border-transparent'
                          )}
                        >
                          {speakingMsgId === msg.id ? '◼' : '🔊'}
                        </button>
                      ...
                      {msg.resources.map((r, i) => (
                        ...
                        >
                          ...
                          <div className="min-w-0">
                            <p className="font-mono text-[0.72rem] text-teal-300 group-hover:text-teal-300 truncate transition-colors">
                              {r.title}
                            </p>
                            {r.description && (
                              <p className="font-mono text-[0.68rem] text-white truncate">{r.description}</p>
                            )}
                          </div>
                          <span className="text-white text-xs ml-auto flex-shrink-0">↗</span>
                        </a>
                      ))}
                    </div>
                  )}
      ...
      {selectedMood && (
        <div className="px-4 pb-1 animate-fade-in">
          <div className="flex items-center gap-2 text-xs">
            <span className="font-mono text-white">Mood set:</span>
          ...
          <div className="flex items-center gap-3 bg-white/4 border border-white/10 rounded-xl px-3 py-2">
            {imageCompressing ? (
              <div className="flex items-center gap-2">
                ...
                <span className="font-mono text-[0.72rem] text-white">Compressing…</span>
              </div>
            ) : pendingImage && (
              <>
                {/* eslint-disable-next-line @next/next/no-img-element */}
                ...
                />
                <span className="font-mono text-[0.72rem] text-white flex-1">Image ready to send</span>
                <button
                  onClick={clearPendingImage}
                  className="text-white hover:text-red-400 text-xs transition-colors"
      ...
      <div className="px-4 pb-6 pt-2 border-t border-white/7 bg-[var(--bg-base)]">
        ...
        {!isUnlimited && usageLeft === 0 ? (
          <div className="bg-white/5 border border-white/10 rounded-2xl p-4 text-center">
            ...
            <p className="font-mono text-[0.72rem] text-white mb-3">
              {userTier === 'scholar'
          ...
          {!isOnline && (
            <div style={{ margin: '0 0 10px', padding: '8px 14px', borderRadius: 10, background: 'rgba(245,158,11,0.08)', border: '1px solid rgba(245,158,11,0.2)', display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <span style={{ fontFamily: 'var(--font-mono)', fontSize: '0.72rem', color: 'rgb(245,158,11)' }}>
          ...
          <div className="flex items-end gap-3">
            ...
            <div className="flex-1 relative">
              <textarea
                ...
                maxLength={2000}
                className={cn(
                  'w-full bg-[var(--bg-surface)] border rounded-2xl px-4 py-3 text-sm text-white placeholder:text-white resize-none outline-none transition-all font-body',
                  sr.isListening
        ...
        <p className="font-mono text-[0.62rem] text-white text-center mt-2">
