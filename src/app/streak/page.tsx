  ...
  return (
    <div className="min-h-screen pb-24" style={{ background: 'var(--bg-base)', position: 'relative', overflow: 'hidden' }}>
      ...
      <div className="max-w-lg mx-auto px-4 py-4 space-y-5">
            ...
            >
              ...
              <div className="px-5 pt-5 pb-0 flex items-center justify-between">
                {tier ? (
                  ...
                  <span className="font-mono text-[0.62rem] text-white/75 uppercase tracking-widest">Daily Streak</span>
                )}
                {streak?.todayDone && (
              ...
              <div className="px-5 pt-3 pb-2 flex items-end gap-3">
                ...
                <div className="pb-3">
                  <div className="font-mono text-sm text-white/82">day{s !== 1 ? 's' : ''}</div>
                  {streak && streak.longestStreak > s && (
                    <div className="font-mono text-[0.62rem] text-white/72 mt-0.5">best: {streak.longestStreak}</div>
                  )}
                </div>
              </div>
              ...
              <div className="px-5 pb-4">
                <div className="font-mono text-[0.62rem] text-white/75 uppercase tracking-widest mb-2">Last 7 days</div>
                  ...
                  {days7.map((done, i) => {
                    ...
                    return (
                      <div key={i} className="flex-1 flex flex-col items-center gap-1">
                        ...
                        >
                          {done ? <span style={{ color: accentColor }}>✓</span> : <span className="text-white/55">·</span>}
                        </div>
                        <span className="font-mono text-[0.62rem] text-white/78">{DAY_LABELS[labelIdx]?.slice(0, 2)}</span>
                      </div>
                    )
                  })}
                </div>
        ...
        {/* ── Milestone unlocks ── */}
        <div>
          <div className="font-mono text-[0.6rem] text-white/78 uppercase tracking-widest mb-3">Milestones</div>
        ...
        {showAdd && (
          <div className="bg-[var(--bg-surface)] border border-teal-600/20 rounded-2xl p-4 space-y-3 animate-fade-up">
            ...
            <input
              ...
              className="w-full bg-white/5 border border-white/10 rounded-xl px-3.5 py-2.5 font-body text-sm text-white placeholder:text-white/75 focus:outline-none focus:border-teal-600/50"
            />

            <div className="grid grid-cols-2 gap-2.5">
              <div>
                <label className="font-mono text-[0.62rem] text-white/78 uppercase tracking-widest block mb-1">Target (R)</label>
                <input
                  type="number" placeholder="5000" value={form.target_amount}
                  onChange={e => setForm(f => ({ ...f, target_amount: e.target.value }))}
                  className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2.5 font-body text-sm text-white placeholder:text-white/75 focus:outline-none focus:border-teal-600/50"
                />
              </div>
              <div>
                <label className="font-mono text-[0.62rem] text-white/78 uppercase tracking-widest block mb-1">Already saved (R)</label>
                <input
            ...
            <div>
              <label className="font-mono text-[0.62rem] text-white/78 uppercase tracking-widest block mb-1">Target date (optional)</label>
              <input
            ...
            {/* Color picker */}
            <div>
              <label className="font-mono text-[0.62rem] text-white/78 uppercase tracking-widest block mb-1.5">Colour</label>
        ...
        {loading ? (
          ...
          <div className="bg-[var(--bg-surface)] border border-white/7 rounded-2xl p-8 text-center">
            ...
            <p className="font-mono text-[0.6rem] text-white/78 mt-1">Set a goal and track your progress</p>
          </div>
        ) : (
          <div className="space-y-3">
            {goals.map(goal => {
                    ...
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex items-center gap-2.5">
                        ...
                        <div>
                          ...
                          {goal.deadline && (
                            <div className="font-mono text-[0.62rem] text-white/78 mt-0.5">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          ...
                          className="font-mono text-[0.62rem] text-white/72 hover:text-red-400 transition-colors px-1"
                        >
                          ✕
                        </button>
                      </div>
                    ...
                    {isContrib && (
                      <div className="mt-3 pt-3 border-t border-white/7 flex gap-2 animate-fade-up">
                        <input
                          ...
                          className="flex-1 bg-white/5 border border-white/10 rounded-xl px-3 py-2 font-body text-sm text-white placeholder:text-white/75 focus:outline-none focus:border-teal-600/50"
                          autoFocus
                        />
                        <button
        ...
        <Link
          href="/budget"
          className="block text-center font-mono text-[0.6rem] text-white/75 hover:text-white/82 transition-colors py-2"
        >
